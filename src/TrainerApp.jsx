...
const postNotification = (d,tk) => {
  ...
  }).then(r=>r.json()).then(j=>console.log('[postNotification]',j)).catch(e=>console.warn('[postNotification] failed',e));
};
...
const TodayScreen=({trainerName,trainerId,token,clients,onViewClient,onTrainerNameUpdated,notifCount,onOpenNotif})=>{
  ...
  const handlePostAnn=async()=>{
    ...
    try{
      ...
      // Broadcast push to all clients — needs the trainer's own token: the server
      // requires it (verifies trainer role before allowing a broadcast).
